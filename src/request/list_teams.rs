...
pub struct ListTeamsRequest<'a> {
    ...
    pub has_members: Option<String>,
    pub has_admins: Option<String>,
    pub page_size: Option<i64>,
    pub next_token: Option<String>,
    pub sort: Option<String>,
}
impl<'a> ListTeamsRequest<'a> {
    pub async fn send(self) -> anyhow::Result<TeamsPaginatedList> {
        let mut r = self.client.client.get("/teams");
        ...
        if let Some(ref unwrapped) = self.organization_id {
            r = r.push_query("organizationId", &unwrapped.to_string());
        }
        if let Some(ref unwrapped) = self.has_members {
            r = r.push_query("hasMembers", &unwrapped.to_string());
        }
        if let Some(ref unwrapped) = self.has_admins {
            r = r.push_query("hasAdmins", &unwrapped.to_string());
        }
    ...
    pub fn has_members(mut self, has_members: &str) -> Self {
        self.has_members = Some(has_members.to_owned());
        self
    }
    pub fn has_admins(mut self, has_admins: &str) -> Self {
        self.has_admins = Some(has_admins.to_owned());
        self
    }
