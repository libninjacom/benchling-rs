...
impl<'a> ListContainersRequest<'a> {
    pub async fn send(self) -> anyhow::Result<ContainersPaginatedList> {
        let mut r = self.client.client.get("/containers");
        ...
        if let Some(ref unwrapped) = self.checkout_status {
            r = r.push_query("checkoutStatus", &unwrapped.to_string());
        }
    ...
    pub fn checkout_status(mut self, checkout_status: &str) -> Self {
        self.checkout_status = Some(checkout_status.to_owned());
        self
    }
