...
pub struct ListRequestFulfillmentsRequest<'a> {
...
impl<'a> ListRequestFulfillmentsRequest<'a> {
    pub async fn send(self) -> anyhow::Result<RequestFulfillmentsPaginatedList> {
        let mut r = self.client.client.get("/request-fulfillments");
        r = r.push_query("entryId", &self.entry_id.to_string());
