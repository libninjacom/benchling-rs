...
pub struct UpdateOligoRequest<'a> {
...
impl<'a> UpdateOligoRequest<'a> {
    pub async fn send(self) -> anyhow::Result<DnaOligo> {
        let mut r = self
            .client
            .client
            .patch(&format!("/oligos/{oligo_id}", oligo_id = self.oligo_id));
...
pub struct UpdateOligoRequired<'a> {
    pub oligo_id: &'a str,
    pub aliases: &'a [&'a str],
