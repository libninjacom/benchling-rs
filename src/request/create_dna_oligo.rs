...
pub struct CreateDnaOligoRequest<'a> {
...
impl<'a> CreateDnaOligoRequest<'a> {
    pub async fn send(self) -> anyhow::Result<DnaOligo> {
        let mut r = self.client.client.post("/dna-oligos");
...
pub struct CreateDnaOligoRequired<'a> {
    pub aliases: &'a [&'a str],
...
impl<'a> CreateDnaOligoRequired<'a> {}
