...
pub struct BulkCreateWorkflowOutputsRequest<'a> {
    pub(crate) client: &'a BenchlingClient,
    pub workflow_outputs: Option<Vec<WorkflowOutputBulkCreate>>,
}
impl<'a> BulkCreateWorkflowOutputsRequest<'a> {
    pub async fn send(self) -> anyhow::Result<AsyncTaskLink> {
        let mut r = self.client.client.post("/workflow-outputs:bulk-create");
    ...
    pub fn workflow_outputs(
        mut self,
        workflow_outputs: Vec<WorkflowOutputBulkCreate>,
    ) -> Self {
