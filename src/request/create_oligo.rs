...
pub struct CreateOligoRequest<'a> {
...
impl<'a> CreateOligoRequest<'a> {
    pub async fn send(self) -> anyhow::Result<DnaOligo> {
        let mut r = self.client.client.post("/oligos");
...
pub struct CreateOligoRequired<'a> {
    pub aliases: &'a [&'a str],
