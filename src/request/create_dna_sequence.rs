...
pub struct CreateDnaSequenceRequest<'a> {
...
impl<'a> CreateDnaSequenceRequest<'a> {
    pub async fn send(self) -> anyhow::Result<DnaSequence> {
        let mut r = self.client.client.post("/dna-sequences");
...
pub struct CreateDnaSequenceRequired<'a> {
    pub aliases: &'a [&'a str],
    pub annotations: Vec<DnaAnnotation>,
...
impl<'a> CreateDnaSequenceRequired<'a> {}
