...
pub struct UpdateRnaOligoRequest<'a> {
...
impl<'a> UpdateRnaOligoRequest<'a> {
    pub async fn send(self) -> anyhow::Result<RnaOligo> {
        let mut r = self
            .client
            .client
            .patch(&format!("/rna-oligos/{oligo_id}", oligo_id = self.oligo_id));
...
pub struct UpdateRnaOligoRequired<'a> {
    pub oligo_id: &'a str,
    pub aliases: &'a [&'a str],
...
impl<'a> UpdateRnaOligoRequired<'a> {}
