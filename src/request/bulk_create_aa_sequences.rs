...
pub struct BulkCreateAaSequencesRequest<'a> {
    pub(crate) client: &'a BenchlingClient,
    pub aa_sequences: Option<Vec<AaSequenceBulkCreate>>,
}
impl<'a> BulkCreateAaSequencesRequest<'a> {
    pub async fn send(self) -> anyhow::Result<AsyncTaskLink> {
        let mut r = self.client.client.post("/aa-sequences:bulk-create");
    ...
    pub fn aa_sequences(mut self, aa_sequences: Vec<AaSequenceBulkCreate>) -> Self {
