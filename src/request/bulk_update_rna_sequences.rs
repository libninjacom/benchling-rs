...
pub struct BulkUpdateRnaSequencesRequest<'a> {
    pub(crate) client: &'a BenchlingClient,
    pub rna_sequences: Option<Vec<RnaSequenceBulkUpdate>>,
}
impl<'a> BulkUpdateRnaSequencesRequest<'a> {
    pub async fn send(self) -> anyhow::Result<AsyncTaskLink> {
        let mut r = self.client.client.post("/rna-sequences:bulk-update");
    ...
    pub fn rna_sequences(mut self, rna_sequences: Vec<RnaSequenceBulkUpdate>) -> Self {
