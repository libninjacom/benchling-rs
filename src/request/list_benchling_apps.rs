...
pub struct ListBenchlingAppsRequest<'a> {
...
impl<'a> ListBenchlingAppsRequest<'a> {
    pub async fn send(self) -> anyhow::Result<BenchlingAppsPaginatedList> {
        let mut r = self.client.client.get("/apps");
