...
pub struct CreateRnaOligoRequest<'a> {
...
impl<'a> CreateRnaOligoRequest<'a> {
    pub async fn send(self) -> anyhow::Result<RnaOligo> {
        let mut r = self.client.client.post("/rna-oligos");
...
pub struct CreateRnaOligoRequired<'a> {
    pub aliases: &'a [&'a str],
...
impl<'a> CreateRnaOligoRequired<'a> {}
