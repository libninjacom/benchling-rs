...
pub struct CreateAaSequenceRequest<'a> {
...
impl<'a> CreateAaSequenceRequest<'a> {
    pub async fn send(self) -> anyhow::Result<AaSequence> {
        let mut r = self.client.client.post("/aa-sequences");
...
pub struct CreateAaSequenceRequired<'a> {
    pub aliases: &'a [&'a str],
...
impl<'a> CreateAaSequenceRequired<'a> {}
