...
pub struct UnarchiveWorkflowTaskGroupsRequest<'a> {
    pub(crate) client: &'a BenchlingClient,
    pub workflow_task_group_ids: Vec<String>,
}
impl<'a> UnarchiveWorkflowTaskGroupsRequest<'a> {
    pub async fn send(self) -> anyhow::Result<WorkflowTaskGroupsArchivalChange> {
        let mut r = self.client.client.post("/workflow-task-groups:unarchive");
        r = r
