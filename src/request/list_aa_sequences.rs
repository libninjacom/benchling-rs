...
pub struct ListAaSequencesRequest<'a> {
...
impl<'a> ListAaSequencesRequest<'a> {
    pub async fn send(self) -> anyhow::Result<AaSequencesPaginatedList> {
        let mut r = self.client.client.get("/aa-sequences");
        ...
        if let Some(ref unwrapped) = self.amino_acids {
            r = r.push_query("aminoAcids", &unwrapped.to_string());
        }
    ...
    pub fn amino_acids(mut self, amino_acids: &str) -> Self {
        self.amino_acids = Some(amino_acids.to_owned());
        self
    }
