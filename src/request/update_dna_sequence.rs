...
pub struct UpdateDnaSequenceRequest<'a> {
...
impl<'a> UpdateDnaSequenceRequest<'a> {
    pub async fn send(self) -> anyhow::Result<DnaSequence> {
        let mut r = self
            .client
            .client
            .patch(
                &format!(
                    "/dna-sequences/{dna_sequence_id}", dna_sequence_id = self
                    .dna_sequence_id
                ),
            );
...
pub struct UpdateDnaSequenceRequired<'a> {
    pub dna_sequence_id: &'a str,
    pub aliases: &'a [&'a str],
    pub annotations: Vec<DnaAnnotation>,
...
impl<'a> UpdateDnaSequenceRequired<'a> {}
