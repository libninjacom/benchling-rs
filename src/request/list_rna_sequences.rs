...
pub struct ListRnaSequencesRequest<'a> {
...
impl<'a> ListRnaSequencesRequest<'a> {
    pub async fn send(self) -> anyhow::Result<RnaSequencesPaginatedList> {
        let mut r = self.client.client.get("/rna-sequences");
        ...
        if let Some(ref unwrapped) = self.bases {
            r = r.push_query("bases", &unwrapped.to_string());
        }
    ...
    pub fn bases(mut self, bases: &str) -> Self {
        self.bases = Some(bases.to_owned());
        self
    }
