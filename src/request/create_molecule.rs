...
pub struct CreateMoleculeRequest<'a> {
...
impl<'a> CreateMoleculeRequest<'a> {
    pub async fn send(self) -> anyhow::Result<Molecule> {
        let mut r = self.client.client.post("/molecules");
...
pub struct CreateMoleculeRequired<'a> {
    pub aliases: &'a [&'a str],
...
impl<'a> CreateMoleculeRequired<'a> {}
